...
class DocumentVector(Base):
    ...
    embedding = Column(Text, nullable=False)  # JSON storage
...
class RecruitmentRAG:
    ...
    def auto_ingest_if_empty(self):
        try:
            ...
            logger.info(f"Document count in DB: {count}")
            ...
            logger.error(f"Auto-ingest check failed: {e}")
    ...
    def ingest_data(self):
        ...
        if not pdf_files:
            logger.warning("No PDFs found. Ingestion aborted.")
            return
...
def manual_ingest():
    bot.ingest_data()
    return {"status": "Manual ingestion completed"}